...
Our two nodes node1 and node2 need to access the Internet. Let's configure a router.
Router's IP is 192.168.45.254/24 and hostname will be "router".

First, we will install iptables-persistent to save iptables rules across reboots:
```bash
user@router:~$ sudo  apt-get install iptables-persistent
```

We add the NAT rule and we update ruleset:
```bash
...
Enable IP forward to enable packets go through interfaces:
...
The router is ready you can use it on node1 and node2:
...
## Configure the recursive DNS server

This server will handle regular DNS requests from clients. It's IP is 192.168.45.1/24 and hostname is dnsrecur. 

Use a temporary DNS server:
```bash
...
We will install the DNS server bind9:
```bash
user@dnsrecur:~$ sudo apt-get install bind9
```

We will configure it as a recursive DNS:
```bash
...
Please check bind9 configuration:
```bash
user@dnsrecur:~$ named-checkconf
```

If it's good, you can restart bind9:
```bash
